<template>
  <div class="app-container">
    ...
    <div class="filter-container">
      <el-input v-model="listQuery.value" placeholder="请输入电影名" style="width: 200px;" class="filter-item" @keyup.enter.native="handleFilter" />
    ...
    <el-table :key="tableKey" v-loading="listLoading" :data="list" border fit highlight-current-row style="width: 100%;" @sort-change="sortChange">
      ...
      <el-table-column label="放映厅" width="110px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.fangyingting }}</span>
        </template>
      </el-table-column>
      <el-table-column label="场次" width="132px" align="center">
        <template slot-scope="{row}">
          <span>{{ row.changci | fmtTime('YYYY-MM-DD HH:MM') }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="座位总数" width="110px" align="center">
        <template slot-scope="{row}">
    ...
    <el-dialog :title="textMap[dialogStatus]" :visible.sync="dialogFormVisible">
      ...
      >
        ...
        <el-form-item label="放映厅" prop="fangyingting">
          <el-select v-model="temp.fangyingting" filterable placeholder="请选择放映厅">
            <el-option
              v-for="(item,index) in fytoptions"
              :key="index"
              :label="item.fangyingting"
              :value="item.fangyingting"
            />
          </el-select>
        </el-form-item>
        ...
        <el-form-item label="已选座位" prop="selected">
          <el-input v-model="temp.selected" />
        </el-form-item>
...
export default {
  ...
  methods: {
    getList() {
      ...
      axios.get('reyingdianying/rydyinfo', { params: this.listQuery }).then((res) => {
        // console.log(res);
    ...
    updateinfo() {
      this.$refs['dataForm'].validate((valid) => {
        ...
        if (valid) {
          axios.post('reyingdianying/updaterydy', tempData).then((res) => {
            // console.log(res);
    ...
    handleDelete(row, index) {
      ...
      axios.post('reyingdianying/del', JSON.stringify(row.id)).then((res) => {
        // console.log(res);
        this.$notify({
          title: 'Success',
          message: '删除成功',
